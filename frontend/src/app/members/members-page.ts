...
import { Invitation, PageResponse, ProjectMember, Role, Task } from '../core/models';
...
import { RemoveMemberDialogComponent, RemoveMemberData } from './remove-member-dialog';
...
export class MembersPageComponent {
  ...
  protected readonly activeByAssignee = computed<Record<string, number | undefined>>(() => {
    ...
    for (const t of this.tasks()) {
      if (t.status !== 'DONE') map[t.assigneeId] = (map[t.assigneeId] ?? 0) + 1;
    }
    return map;
  });
  ...
  protected invite(): void {
    ...
    if (!p) return;
    this.dialog
      .open<Invitation | undefined>(InviteDialogComponent, { hasBackdrop: true, data: { projectId: p.id } })
      .closed.subscribe((inv) => {
        if (inv) this.invitations.update((list) => [inv, ...list]);
      });
  }
